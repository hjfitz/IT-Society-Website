...
const print = msg => utility.print(prefix)(msg);
...
contentfulApi.get('/all', async (req, res) => {
  ...
  print(
    `Returning ${entries.items
      .length} entries @ /api/contentful/all in conentful/index.js`,
  );
...
contentfulApi.get('/committeeMembers', async (req, res) => {
  const entries = await client.getEntries();
  print(
    'Returning all Committee Members @ /api/contentful/committeeMembers in contentful/index.js',
  );
...
contentfulApi.get('/committee/:year', async (req, res) => {
  ...
  const specificYear = committee.filter(
    specificCommittee => specificCommittee.fields.committeeYear === year,
  );
  print(
    `Returning Committee Members of year ${year} @ /api/contentful/committee/:year in contentful/index.js`,
  );
...
contentfulApi.get('/events', async (req, res) => {
  ...
  print(
    `Returning ${events.length} events @ /api/contentful/events in contentful.index.js`,
  );
  res.json(events);
});
